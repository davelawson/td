...
The current phase is still early prototype foundation. Peaceful play is split into immediate post-Raid Labour and player-ended Management. A new game begins in Management on sparse generated grasslands, where exploration presents a preassigned grasslands, hills, or Forest biome beside each free adjacent-Plot reveal button. Home and explored grasslands are biased toward sparse Trees, hills is biased toward sparse Boulders and more Iron Deposits, and Forest is heavily Tree-biased with a few Boulders but no Iron Deposits. Tower and economic building construction requires and reserves available inhabitants. House construction provides the first population-seeding path by converting Wood into Peasants, Barracks construction converts Peasants into Soldiers, and Dorm construction converts a Peasant into an Apprentice. Woodcutter, Stone Quarry, and Iron Mine provide Labour-phase production by consuming the nearest matching Tree, Boulder, or Iron Deposit anywhere in the explored Domain; exhausted producers wait for later matching terrain. Northward exploration extends the current Raid path, and all exploration can expose future production inputs. Raids scale deterministically with Raid number, total settlement population, and every explored Plot, tying expansion and growth to both defensive pressure and resource availability. Broader recruitment, terrain regrowth, and richer gathering remain missing capabilities, but every current tower staffing role now has a normal-play source.
