...
The completed foundation plans have established the runnable app, expanded menu flow, menu package boundary, New Game configuration screen, current 1920x1080 resize policy, first game-state package, app-mode transition, logical update counter, and pause behavior. The next priorities should remain separate plans so the codebase grows from verified behavior rather than speculative architecture.
