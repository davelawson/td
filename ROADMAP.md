...
The completed foundation plans have established the runnable app, expanded menu flow, menu package boundary, New Game configuration screen, current 1920x1080 resize policy, first game-state package, app-mode transition, logical update counter, pause behavior, in-game overlay menu, a prototype top bar for future gameplay status, a static starting Plot scene backed by prototype map state, basic camera zoom and pan for scene inspection, and first selected-object state for structures and raiders. The next priorities should remain separate plans so the codebase grows from verified behavior rather than speculative architecture.

1. Add the next basic scene interaction. Camera movement and selected-object state are now the first implemented inspection models; a future small slice can add a lightweight object detail panel before adding placement, upgrades, or resource rules.
