...
- Building and command bars should stay screen-space, compact, and visually subordinate to the map. Build affordances must only imply actions that are implemented; the current building bar supports informational hover tooltips, affordability-aware hover, and a simple half-sized dragged icon for Management-phase placement, but not upgrades, selling, range previews, or broader construction commands. The top HUD should name Management plainly; the instantaneous Labour phase needs no transient panel or animation in the current slice.
