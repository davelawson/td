...
- Static map scenes should keep tile boundaries, roads, and the Sanctum readable before adding decorative terrain or production art. The home Plot and explored grasslands use the same sparse Tree, Boulder, and Iron Deposit generation, so the starting view should read as grasslands without obscuring its Sanctum, protected north road, exploration controls, or buildable empty grass. Generated hills should read as more Boulder-heavy than grasslands and make rare Iron Deposits noticeably more likely while remaining mostly buildable. Forest should read as substantially denser and Tree-dominant, with occasional Boulders, no Iron Deposits, and enough empty grass to keep construction viable. Shared explored-Plot joins and protected roads should remain visually open in every biome. Boulder and Iron Deposit terrain should use transparent terrain sprites consistent with the pine-tree assets. Iron Deposits should share the ordinary Boulders' gray-rock silhouette language and scale while remaining distinguishable through sparse, high-contrast blue ore veins at 50% scale.
- Exploration affordances should be legible as map controls. The current magnifying-glass buttons sit on borders between explored and unexplored orthogonal Plots, and they should remain visually distinct from Tiles, structures, roads, and screen-space UI. Each button's preassigned `Grasslands`, `Hills`, or `Forest` label should use compact fixed-size text placed outward into unexplored space: above north, right of east, below south, and left of west. The label is informational; only the circular magnifying-glass is clickable.
...
- Selected raiders and structures should have a plainly brighter visual state that is easy to notice without obscuring the sprite, health bar, road, or Tile context. Selected Tree, Boulder, and Iron Deposit terrain should use a gold Tile outline so the terrain sprite and biome remain readable. A selected combat tower should additionally show its exact maximum range as a camera-scaled Light Bronze circle: use a faint translucent fill for coverage and a three-pixel opaque boundary, while drawing later controls, raiders, projectiles, and screen-space UI above it.
- Selected-object detail panels should stay compact, anchored in screen space, and readable. When a selected object has implemented commands, keep those controls visually connected to the object without turning every inspection panel into a command surface. The current Market uses a fixed-size vertical button group beside its projected Tile, falls back to the opposite side near an edge, stays clear of persistent UI, and hides when the Market is off-screen or trading is unavailable.
- Building and command bars should stay screen-space, compact, and visually subordinate to the map. Build affordances must only imply actions that are implemented; the current building bar appears only during Management and supports informational hover tooltips, affordability-aware hover, and a simple half-sized dragged icon for placement, but not upgrades, selling, placement range previews, or broader construction commands. During Raid and breach, the hidden bar must not reserve an invisible input region. The top HUD should name Management plainly and append the next challenge preview; Raid status should retain the generated challenge. The instantaneous Labour phase needs no transient panel or animation in the current slice.
- Camera zoom and pan should affect the map scene only. Right-drag panning should start only from the game view, not from the top HUD, building bar, Market buttons, selection panel, or overlay. Contextual controls may follow a projected map anchor but should retain fixed screen-pixel hit targets. The top HUD, debug counter, pause label, and in-game overlay should remain readable screen-space UI, and the overlay should block camera controls while it is open.
