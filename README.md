...
The repository now contains a small runnable prototype shell: a Go/Ebitengine desktop app with a main menu, Wizard name entry, a Sanctum-only starting Plot, camera inspection, object selection, resource and population status, and a building bar for House, Barracks, plus Bow, Flame Bolt, and Catapult Towers. House construction costs Wood and adds Peasants. Barracks construction costs Wood and Stone and converts Peasants into Soldiers. Tower construction requires both the displayed resources and available staff; successful construction reserves staff by reducing available counts while totals remain unchanged. Deterministic Raids, projectile combat, Catapult Tile-area damage, combat rewards, pause, an in-game overlay, and quit behavior are also implemented. Timed recruitment and staff release are not implemented.

## Current Status

- Stage: local runnable prototype foundation.
- Runtime stack: Go with Ebitengine.
- Current playable slice: a new game starts with only the Sanctum, 100 Wood, 50 Stone, 20 Metal, and `0/0` for Apprentices, Soldiers, and Peasants. House costs 20 Wood, requires no staff, and immediately adds 2 available and total Peasants. Barracks costs 10 Wood and 10 Stone, consumes 2 available and total Peasants, and adds 2 available and total Soldiers. Bow requires one available Soldier, Flame Bolt requires one available Apprentice, and Catapult requires one available Soldier plus two available Peasants. An icon highlights and starts dragging only when resources and any required population or staff are sufficient. Successful tower placement deducts resources and reduces the required roles' available counts without changing totals. Existing camera, selection, Raid, combat, pause, overlay, and menu behavior remains available.
...
- Current non-goals: exploration, resource gathering, timed inhabitant recruitment and assignment, staff release, tower upgrades, broader base-building, full tower-defense combat, saving games, and campaign structure.
