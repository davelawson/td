...
The repository now contains a small runnable prototype shell: a Go/Ebitengine desktop app with a main menu, Wizard name entry, a Sanctum-only starting Plot, camera inspection through mouse-wheel zoom, `WASD` panning, and right-drag panning, object and natural-obstacle selection, resource and population status, and a 260-pixel tabbed building bar with each building's values shown to the right of its icon. Peaceful play is split into an instantaneous Labour phase after each successful Raid and an open-ended Management phase. During Management the player can explore and construct until clicking `Next Raid`; during Labour placed economic buildings consume terrain to produce resources. The building bar appears only during Management. Hovering a building icon opens a tooltip with its description, cost, staffing requirement, and implemented effect or combat stats. The bar defaults to `Housing` for House, Barracks, and Dorm, with `Economic` for Woodcutter, Stone Quarry, and Iron Mine, and `Defenses` for Bow, Flame Bolt, and Catapult Towers. House construction costs Wood and adds Peasants. Barracks construction costs Wood and Stone and converts Peasants into Soldiers. Dorm construction costs Wood and Stone and converts a Peasant into an Apprentice. Woodcutter, Stone Quarry, and Iron Mine construction reserves one Peasant; each Labour they consume the nearest Tree, Boulder, or Iron Deposit anywhere in the explored Domain before producing Wood, Stone, or Metal. A producer without matching terrain produces nothing. Tower construction requires both the displayed resources and available staff; successful construction reserves staff by reducing available counts while totals remain unchanged. Buildable icon squares have green outlines, while buildings without sufficient resources, population, or staff have red outlines and render their icons at 70% opacity. The centered top-bar status previews the next Raid challenge during Management and retains the generated challenge during a Raid. Trees, Boulders, and rare Iron Deposits can be selected to show their terrain type and Plot biome, while empty grass and Road cannot. Consumed terrain becomes the biome's default grass. Deterministic scaling Raids derive their contents from Raid number, settlement population, and explored Plots; their sprite-backed roster contains Skeletons, Zombies, fast Ghouls, and durable Armoured Skeletons. Projectile combat, Catapult Tile-area damage, combat rewards, pause, an in-game overlay, and quit behavior are also implemented. Timed recruitment, worker reassignment, staff release, and terrain regrowth are not implemented.

## Current Status

- Stage: local runnable prototype foundation.
- Runtime stack: Go with Ebitengine.
- Current playable slice: a new game starts in Management with only the Sanctum, 100 Wood, 50 Stone, 20 Metal, and `0/0` for Apprentices, Soldiers, and Peasants. The map camera supports mouse-wheel zoom, `WASD` panning, and right-drag panning that starts only over the game view rather than visible screen-space UI. Trees, Boulders, and Iron Deposits can be selected for terrain and biome details with a gold Tile outline; empty grass and Road are not selectable. Grasslands generate 1% Iron Deposit and hills generate 3%; all three natural terrain types block construction and serve as exhaustible Labour inputs. The Management-only building bar starts on `Housing`, and its tabs switch between Housing, Economic, and Defenses choices. House costs 20 Wood, requires no staff, and immediately adds 2 available and total Peasants. Barracks costs 10 Wood and 10 Stone, consumes 2 available and total Peasants, and adds 2 available and total Soldiers. Dorm costs 10 Wood and 10 Stone, consumes 1 available and total Peasant, and adds 1 available and total Apprentice. Woodcutter costs 10 Wood, Stone Quarry costs 10 Wood and 10 Stone, and Iron Mine costs 10 Wood, 10 Stone, and 10 Metal; each reserves one available Peasant and adds 10 of its matching resource during Labour only when it can consume the nearest matching terrain in the explored Domain. Bow costs 20 Wood and 10 Stone and requires one available Soldier, Flame Bolt requires one available Apprentice, and Catapult requires one available Soldier plus one available Peasant. Values are displayed to the right of each building icon, and icon hover tooltips show longer descriptions plus costs, staffing, production, population effects, or tower combat stats. An icon square has a green outline and highlights only when resources and any required population or staff are sufficient; otherwise its square has a red outline and the icon is drawn at 70% opacity. Successful staffed placement deducts resources and reduces the required roles' available counts without changing totals. The top bar previews the next challenge during Management. Starting a Raid freezes that deterministic challenge from its number, total population, and all explored Plots, hides the building bar, and releases Skeletons, Zombies, Ghouls, and Armoured Skeletons at score multiples of 2, 4, 6, and 8. Challenge 4 still produces Raid 1's two Skeletons and one Zombie. Existing combat, pause, overlay, and menu behavior remains available.
