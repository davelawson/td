...
The game design is intentionally early. The current implementation splits peaceful play into an instantaneous Labour phase after each successful Raid and an open-ended Management phase. A new game begins in Management. Management allows free exploration of adjacent Plots and construction until the player clicks `Next Raid`; Labour lets each placed economic building consume one matching terrain Tile before immediately entering Management. The prototype also includes a Sanctum-only starting Plot, a House that adds Peasant population, a Barracks that converts Peasants into Soldiers, a Dorm that converts a Peasant into an Apprentice, and tower templates with resource and staffing requirements. The home Plot uses the same sparse grasslands terrain generation as explored Grasslands; each unexplored frontier Plot independently receives Grasslands, Hills, or Forest with near-equal probability before exploration and displays that assignment beside its explore button. Every generated Plot is immediately usable and mostly empty grass: Grasslands is sparsely Tree-biased, Hills is Boulder-biased with more Iron Deposits, and Forest is heavily Tree-biased with a few Boulders but no Iron Deposits. Exploring the central north chain extends the current visible road and Raid path, while exploring in any direction exposes potential production inputs. Economic building and tower construction requires sufficient available staff and reserves those inhabitants. Timed recruitment, reassignment, staff release, and terrain regrowth are not implemented.
...
The first object-inspection interaction is left-click selection. Active raiders have first priority, structure Tiles have second priority, and natural Tree, Boulder, or Iron Deposit terrain has third priority. Empty grass and Road are not selectable. A selected raider or structure is drawn brighter; selected terrain receives a gold Tile outline. Selecting a combat tower also draws a camera-scaled faint gold coverage circle centered on its Tile, with an opaque edge at the same maximum center-to-center range used for targeting. Raiders show combat stats and Gold drops; structures and terrain show facts appropriate to their type. Market is the first selected object with commands: during Management, three fixed-size buttons appear beside its projected Tile for one-unit purchases. Other selections remain informational and there are no upgrades.
...
The first implemented dominant character is a biome. A biome is a stored Plot label that controls how terrain is generated when the Plot is explored. The current biomes are `grasslands`, `hills`, and `forest`. The starting home Plot and explored grasslands both generate 6% Tree, 3% Boulder, 1% Iron Deposit, and 90% empty Grass; the home Plot then overwrites generated terrain with its Sanctum road. When an unexplored Plot first becomes orthogonally adjacent to explored land, it independently receives Grasslands for rolls 0-32, Hills for 33-65, or Forest for 66-99. The stable assignment is displayed beside its explore button before terrain is revealed; only the circular button explores. Hills generates 3% Tree, 6% Boulder, 3% Iron Deposit, and 88% empty Grass. Forest generates 18% Tree, 3% Boulder, no Iron Deposit, and 79% empty Grass. Road and shared-edge rules can overwrite generated terrain so roads stay readable and adjacent explored Plots join cleanly. Trees, Boulders, and Iron Deposits block construction and are exhaustible inputs for Woodcutters, Stone Quarries, and Iron Mines. Consumption replaces each with its biome's default terrain, which is grass in all three implemented biomes.
...
Tower-defense encounters should use clear enemy movement, clear defensive coverage, and visible and audible combat results. The first Raid slice uses one simple fixed path with Skeleton, Zombie, Ghoul, and Armoured Skeleton enemies. The first combat slice adds automated Bow, Flame Bolt, and Catapult Towers with testable targeting, projectile travel, damage rules, Catapult Tile-area impact, selected-tower range visualization, and a prototype sound effect for tower-damage defeats without replacing the simple Raid lifecycle.
...
The centered in-game top bar shows the next Raid's live challenge preview during Management. During a Raid it shows both enemies remaining and the generated Raid's frozen challenge, and breach retains that frozen rating. Challenge is formatted with one decimal place.
...
- The first gameplay-facing rendered slice starts in Management with a sparse generated-grasslands home Plot containing only the centered Sanctum as an initial structure, a protected straight road north, free exploration buttons on unexplored adjacent Plot borders, Tree, Boulder, and Iron Deposit inspection, an upcoming challenge preview, and a widened tabbed building bar listing Housing, Economic, and Defenses groups with right-side costs, population costs, population grants or staffing requirements, informational hover tooltips, green/red capacity outlines, capacity opacity, and Management-phase drag placement. Raid and breach hide the building bar and retain the frozen generated challenge.
...
- The first exploration slice uses free magnifying-glass border buttons during Management, including paused Management, to reveal orthogonally adjacent Plots. Each frontier Plot independently receives a stable Grasslands, Hills, or Forest assignment before exploration and displays its name outward beside the button; the label is informational and only the circle reveals. Grasslands uses 6% Tree, 3% Boulder, 1% Iron Deposit, and 90% empty grass; Hills uses 3% Tree, 6% Boulder, 3% Iron Deposit, and 88% empty grass; Forest uses 18% Tree, 3% Boulder, no Iron Deposit, and 79% empty grass. The first implementation collapses scouting and claiming into one action.
...
- Decision: Allow inspection of Tree, Boulder, and Iron Deposit terrain, but not empty grass or Road, after raider and structure selection priority.
  Rationale: Natural obstacles are meaningful visible subjects whose terrain and biome context helps map inspection, while selecting every empty or route Tile would make ordinary clicks noisy. A gold Tile outline identifies the selected obstacle without obscuring its sprite.
  Date/Author: 2026-07-14 / User and Codex
...
- Decision: Show a selected combat tower's exact maximum range as a faint gold filled circle with an opaque boundary.
  Rationale: Using the same center-to-center radius as combat makes defensive coverage inspectable during every phase without obscuring the map. This does not add a placement preview before construction.
  Date/Author: 2026-07-14 / User and Codex
...
- Decision: Let economic buildings consume the nearest matching terrain anywhere in the explored Domain, processing buildings and equal-distance candidates in deterministic map order. Consumed terrain becomes the biome default; Grasslands, Hills, and Forest all become grass.
  Rationale: Domain-wide reach makes every explored Plot relevant to production without adding worker paths or range UI. Nearest selection makes visible depletion understandable, while stable ordering keeps scarce inputs and tests reproducible.
  Date/Author: 2026-07-14 / User and Codex
...
- Decision: Show the building bar only during Management and release its screen region to map input during Raids and after breach.
  Rationale: Construction is a Management action, so retaining either visible controls or an invisible input blocker during combat misrepresents the available workflow and reduces usable map space.
  Date/Author: 2026-07-14 / User and Codex
...
- Decision: Generate deterministic scaling Raid templates from Raid number, total settlement population, and all explored Plots.
  Rationale: `1.2^(raidNumber-1) * 1.2^(plotsExplored-1) * (1 + population/10) + 3` connects progression, growth, and expansion to pressure. A progress window releases enemies at type-specific score multiples, giving the template capacity to scale without modifying individual enemy stats.
  Date/Author: 2026-07-14 / User and Codex

- Decision: Extend generated Raids with Ghouls at score multiples of 6 and Armoured Skeletons at score multiples of 8, after the existing Skeleton and Zombie rules.
  Rationale: The fast 40-health Ghoul and durable 125-health Armoured Skeleton add readable pressure roles to later challenge scores while challenge 4 still yields Raid 1's accepted two Skeletons and one Zombie.
  Date/Author: 2026-07-14 / User and Codex

- Decision: Scale the Raid release window as `5 + 2 * sqrt(challenge)` seconds.
  Rationale: The square-root curve preserves the accepted nine-second first Raid while making later, larger rosters spawn increasingly quickly instead of extending Raid length almost in proportion to challenge.
  Date/Author: 2026-07-14 / User and Codex

- Decision: Display challenge with one decimal place beside centered phase status, previewing the next generated value during Management and freezing the current value during Raid and breach.
  Rationale: Immediate preview makes settlement and exploration pressure legible before commitment, while the stored Raid template remains the authoritative description after an assault begins.
  Date/Author: 2026-07-14 / User and Codex
...
- Decision: Store a biome on each Plot and use grasslands as the first generated biome.
  ...
  Date/Author: 2026-07-13 / Codex
- Decision: Add Boulder as non-buildable generated terrain. Revised on 2026-07-14 so Stone Quarries consume Boulders during Labour.
  Rationale: This provides a readable obstacle and finite Stone-production input while retaining empty-grass placement for the Quarry.
  Date/Author: 2026-07-13 / Codex
...
- Decision: Generate the home Plot with the same biome weights as explored grasslands, without a guaranteed Sanctum clearing. The weights were extended on 2026-07-14 to 6% Tree, 3% Boulder, 1% Iron Deposit, and 90% empty Grass.
...
- Decision: Choose Grasslands, Hills, or Forest independently with near-equal probability when a non-home Plot first joins the exploration frontier.
  Rationale: Frontier-time assignment fixes the biome before exploration and supports an informed destination choice without generating hidden terrain or introducing a whole-map seed. Percentage rolls assign 33% each to Grasslands and Hills and the remaining 34% to Forest.
  Date/Author: 2026-07-14 / User and Codex
...
- Decision: Bias hills toward Boulder. The weights were extended on 2026-07-14 to 3% Tree, 6% Boulder, 3% Iron Deposit, and 88% empty Grass.
...
- Decision: Add Forest at 18% Tree, 3% Boulder, no Iron Deposit, and 79% empty Grass.
  Rationale: Forest provides a clearly Wood-oriented exploration choice with a few Stone inputs and no native Iron supply while remaining mostly buildable.
  Date/Author: 2026-07-14 / User and Codex

- Decision: Add Iron Deposit as rare non-buildable terrain at 1% in grasslands and 3% in hills. Revised on 2026-07-14 so Iron Mines consume deposits during Labour without requiring deposit-based placement.
  Rationale: This makes exploration reveal a scarce, biome-sensitive production input while keeping placement on ordinary grass and avoiding a separate extraction or assignment interface.
  Date/Author: 2026-07-14 / User and Codex
...
- Decision: Keep the first rendered home Plot empty except for the Sanctum and north road. Superseded on 2026-07-14 by ordinary grasslands generation on the home Plot.
  Rationale: The original milestone proved map state and static scene rendering before terrain rules existed; generated Tree, Boulder, and Iron Deposit terrain is now implemented and belongs on the home Plot too.
  Date/Author: 2026-05-13 / Codex
