...
The first implemented dominant character is a biome. A biome is a stored Plot label that controls how terrain is generated when the Plot is explored. The current biomes are `grasslands` and `hills`. The starting home Plot and explored grasslands both generate 6% Tree, 3% Boulder, 1% Iron Deposit, and 90% empty Grass; the home Plot then overwrites generated terrain with its Sanctum road. When an unexplored Plot first becomes orthogonally adjacent to explored land, it independently has an equal chance to be assigned grasslands or hills. The stable assignment is displayed beside its explore button before terrain is revealed; only the circular button explores. Hills generates 3% Tree, 6% Boulder, 3% Iron Deposit, and 88% empty Grass. Road and shared-edge rules can overwrite generated terrain so roads stay readable and adjacent explored Plots join cleanly. Trees, Boulders, and Iron Deposits block construction and are exhaustible inputs for Woodcutters, Stone Quarries, and Iron Mines. Consumption replaces each with its biome's default terrain, which is grass in both implemented biomes.
...
- Decision: Let economic buildings consume the nearest matching terrain anywhere in the explored Domain, processing buildings and equal-distance candidates in deterministic map order. Consumed terrain becomes the biome default; grasslands and hills both become grass.
