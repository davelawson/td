...
7. While unpaused, each Ebitengine update advances the logical update counter by one. If a Raid is active, the game package advances deterministic skeleton spawning, Bow Tower targeting and projectile combat, and enemy movement along the fixed north road. Enemies store Sanctum-centered world coordinates in Tile units, have health, and render from the skeleton sprite in the active asset catalog. The Bow Tower stores timing stats in seconds, tracks cooldowns as seconds remaining, launches arrow projectiles at in-range enemies, and damages enemies on projectile impact. Enemies that reach the Sanctum spend Barricade charges and are removed, or breach the Sanctum when no charges remain.
...
To add gameplay systems, start with pure data and functions that can be tested by `go test ./...`, then connect them to Ebitengine rendering and input. The current prototype map, camera, coordinates, Raid, and combat systems follow that pattern: `internal/game/map.go` creates the default home Plot, stores per-Tile visual variation tweaks, `internal/game/coordinates.go` maps grid Tiles into Sanctum-centered world positions, `internal/game/camera.go` owns camera movement and projection state, `internal/game/raid.go` owns deterministic Raid state transitions and enemy world positions, `internal/game/combat.go` owns Bow Tower targeting, cooldown, projectile, and damage rules, and rendering reads from stored map, camera, Raid, and combat state.
