...
7. While unpaused, each Ebitengine update advances the logical update counter by one. If a Raid is active, the game package advances deterministic skeleton spawning and movement along the fixed north road. Enemies render from the skeleton sprite in the active asset catalog. Enemies that reach the Sanctum spend Barricade charges and are removed, or breach the Sanctum when no charges remain.
