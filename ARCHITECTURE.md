...
7. The game package renders a top bar with prototype Chapter, Day, resources, phase, and Sanctum barricade status, plus a bottom-left `Next Raid` button, selected-object detail panel, and debug logical update counter in screen space so camera changes affect only the map scene.
