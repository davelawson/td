...
7. The game package renders resources and available/total populations. Exploration buttons are handled as map-space controls but are blocked by screen-space UI. The building bar groups visible choices into `Housing`, `Economic`, and `Defenses` tabs, displays values to the right of each icon, shows informational tooltips when building icons are hovered, draws buildable icon slots with green outlines, draws capacity-blocked icon slots with red outlines and 70% icon opacity, and uses stable building identifiers for placement mapping. Building-bar eligibility requires sufficient resources and all required available roles. Successful staffed placement deducts resources and reserves staff atomically; successful House placement deducts Wood and grants Peasants by increasing both available and total counts; successful Barracks placement deducts Wood and Stone, removes Peasants from available and total counts, and grants Soldiers by increasing both available and total counts; successful Dorm placement deducts Wood and Stone, removes one Peasant from available and total counts, and grants one Apprentice by increasing both available and total counts; successful economic building placement reserves one available Peasant and produces its resource during each later Labour phase. A new game starts at `0/0` for every role, so House is initially eligible but Barracks, Dorm, economic buildings, and towers are not.
...
To add gameplay systems, start with pure data and functions that can be tested by `go test ./...`. `internal/game/structures.go` owns staffing requirements, population-cost, population-grant, resource-yield, and description template metadata; `internal/game/hud.go` owns population availability, population costs, reservation, grants, and phase display; `internal/game/phase.go` owns the post-Raid Labour-to-Management transition; `internal/game/resources.go` applies economic building yields during Labour; `internal/game/building_bar.go` owns building-bar rendering, tab input, and drag behavior; `internal/game/building_bar_items.go` owns stable building-bar item and category mapping; `internal/game/building_bar_metadata.go` owns compact cost and population metadata display helpers; and `internal/game/building_tooltip.go` owns detailed hover tooltip formatting and rendering. A future removal or reassignment system must return reserved staff without duplicating template requirements, and must define how removing a population-provider, converter, or economic building affects inhabitants and Labour income before changing House, Barracks, or economic building behavior.
