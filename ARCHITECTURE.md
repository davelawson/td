...
- `internal/game/` owns game status, tower templates, construction resources, available/total populations, staffing checks and reservations, placement, combat, rendering, and input behavior. Staffing reservation is part of successful construction; no separate assignment subsystem exists yet.
...
7. The game package renders resources and available/total populations. Building-bar eligibility requires both sufficient resources and all required available roles. Successful placement deducts resources and reserves staff atomically; population totals remain unchanged. A new game starts at `0/0` for every role, so no tower is initially eligible.
...
To add gameplay systems, start with pure data and functions that can be tested by `go test ./...`. `internal/game/structures.go` owns staffing requirements, `internal/game/hud.go` owns population availability and reservation, and `internal/game/building_bar.go` applies both resource and staffing gates at drag start and release. A future removal or reassignment system must return reserved staff without duplicating template requirements.
