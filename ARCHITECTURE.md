...
3. The game package renders explored 15x15 Plots from its stored prototype map data. A new game starts with only the home Plot explored. The home Plot is open grassland containing the centered Sanctum as its only initial structure and a straight road north to the Plot edge. Magnifying-glass buttons appear on borders between explored and unexplored orthogonal Plots during calm play. Clicking one reveals the connected Plot as usable grassland; adjacent explored Plots have no plot-level frame, gutter, or padding between them; north-chain Plots also continue the straight center road. All combat towers must be built by the player.
...
- Keep the explored Plot map data in `internal/game/map.go` and the first camera/projection behavior, including right-drag camera state, in `internal/game/camera.go` until generation, richer multi-map behavior, or multiple gameplay scenes create a reason for separate packages.
