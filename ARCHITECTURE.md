...
7. The game package renders resources, available/total populations, and centered phase-plus-challenge status. During Management the challenge is regenerated as a live preview from the next Raid number, total population, and explored Plot count; during Raid and breach it comes from the frozen Raid template. Exploration buttons are handled as map-space controls but are blocked by visible screen-space UI. The building bar appears only during Management, groups visible choices into `Housing`, `Economic`, and `Defenses` tabs, displays values to the right of each icon, shows informational tooltips when building icons are hovered, draws buildable icon slots with green outlines, draws capacity-blocked icon slots with red outlines and 70% icon opacity, and uses stable building identifiers for placement mapping. When hidden, the bar does not block map input. Building-bar eligibility requires sufficient resources and all required available roles. Successful staffed placement deducts resources and reserves staff atomically; successful House placement deducts Wood and grants Peasants by increasing both available and total counts; successful Barracks placement deducts Wood and Stone, removes Peasants from available and total counts, and grants Soldiers by increasing both available and total counts; successful Dorm placement deducts Wood and Stone, removes one Peasant from available and total counts, and grants one Apprentice by increasing both available and total counts; successful economic building placement reserves one available Peasant. During each later Labour, every Woodcutter, Stone Quarry, and Iron Mine attempts to consume the nearest matching Tree, Boulder, or Iron Deposit in any explored Plot before producing its resource. A new game starts at `0/0` for every role, so House is initially eligible but Barracks, Dorm, economic buildings, and towers are not.
