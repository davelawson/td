...
7. The game package renders a top bar with prototype Chapter, Day, resources, phase, and Sanctum barricade status, a visual-only building bar on the left edge below the top bar, a bottom-left `Next Raid` button beside that bar, selected-object detail panel, and debug logical update counter in screen space so camera changes affect only the map scene.
