...
- `internal/game/` owns the first top-level game state, Wizard name storage, pause state, logical update counting, active asset catalog ownership, prototype enemy and structure catalog ownership including construction costs and enemy defeat rewards, gameplay sound-event reporting through a narrow sink interface, prototype top-bar status display with sprite-backed resource icons and fixed Apprentice/Soldier/Peasant available/total values, building bar with affordability-aware hover icon and cost rendering for Bow, Flame Bolt, and Catapult Towers, calm-phase build-drag state, tower placement validation, resource-cost deduction, prototype map data, Sanctum-centered Tile-unit world coordinates, prototype camera state, map-to-screen projection, selected structure and raider state, selected-object detail-panel formatting and rendering, home Plot rendering, deterministic placeholder Raid state, tower targeting and projectile state, combat-defeat resource payout, sprite-backed enemy and projectile rendering, Catapult Tile-area projectile impact, bottom-left in-game Raid controls, and in-game overlay menu behavior. Its `State` type stays the package root while private substructs group prototype game-status data separately from assets, map data, camera state, selected item state, build drag state, Raid state, combat state, scene rendering layout, font faces, and overlay UI state. It may later grow into exploration, inhabitant recruitment and assignment, broader base-building, and fuller defense scene state when those systems exist.
...
3. The game package renders a static 15x15 home Plot from its stored prototype map data. The Plot contains the centered Sanctum, rendered from a loaded sprite, a straight road north to the Plot edge, Bow and Flame Bolt Towers rendered from loaded sprites across the road from each other, and pine trees around the Plot edge except where the road exits. Catapult Tower is available from the building bar but is not placed for free in the starting Plot.
...
7. The game package renders a top bar with prototype Chapter, Day, sprite-backed Wood/Stone/Metal resource counts, a separate sprite-backed Apprentice/Soldier/Peasant group formatted as available/total, phase, and Sanctum barricade status, a building bar on the left edge below the top bar with Bow Tower, Flame Bolt Tower, and Catapult Tower icons and colour-coded construction costs, a bottom-left `Next Raid` button beside that bar, selected-object detail panel, and debug logical update counter in screen space so camera changes affect only the map scene. The population values are private fixed game-status data until recruitment and assignment systems exist. Hovering an affordable building-bar icon brightens that icon and emphasizes only its matching cost row.
8. Left-dragging an affordable building-bar icon during calm play starts a private build drag. A half-sized copy of the icon follows the cursor. Releasing over an empty `terrainEmpty` Tile with no feature places the matching tower feature and deducts its cost; releasing over occupied Tiles, roads, forests, during active Raids, or after breach cancels without spending resources. SPACE-paused calm play still permits placement, while the in-game overlay blocks building input.
...
To add gameplay systems, start with pure data and functions that can be tested by `go test ./...`, then connect them to Ebitengine rendering and input. The current prototype map, camera, coordinates, selection, Raid, and combat systems follow that pattern: `internal/game/map.go` creates the default home Plot, stores per-Tile visual variation tweaks, `internal/game/coordinates.go` maps grid Tiles into Sanctum-centered world positions, `internal/game/camera.go` owns camera movement and projection state, `internal/game/structures.go` owns structure template stats and construction costs, `internal/game/enemies.go` owns enemy template stats and defeat-resource rewards, `internal/game/selection.go` owns selected structure and raider hit testing, `internal/game/selection_panel.go` owns selected-object detail-panel formatting, rendering, and click blocking, `internal/game/building_bar.go` owns the tower icon bar, construction-cost rendering, affordability hover, build-drag state, placement validation, and cost spending, `internal/game/raid.go` owns deterministic Raid state transitions and enemy world positions, `internal/game/combat.go` owns tower targeting, cooldown, projectile, single-target damage, Catapult Tile-area damage, and combat-defeat reward rules, and rendering reads from stored map, camera, selection, build drag, Raid, and combat state.
