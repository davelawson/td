...
The repository contains an early runtime shell: a Go module, a small Ebitengine executable, an asset catalog package that loads runtime sprites, HUD icons, and audio bytes, a small runtime sound manager, a menu package that owns the current menu flow, a UI package with shared presentation helpers, and a game package that owns the first logical game state, prototype explored-Plot map state with grasslands biome generation, Sanctum-centered world coordinates, camera state with keyboard and right-drag panning, selected structure and raider state, selected-object detail data, widened tabbed building bar with right-side construction-cost and population metadata display, building hover tooltips, green/red capacity outlines, capacity opacity, affordable-option icon hover feedback, free calm-phase adjacent Plot exploration, calm-phase structure drag placement, House Peasant grants, Barracks Peasant-to-Soldier conversion, Dorm Peasant-to-Apprentice conversion, economic building Peasant reservation and post-Raid resource production, explored Plot projection and rendering, deterministic placeholder Raid state with sprite-backed skeleton and zombie enemies, first-pass tower projectile combat including Catapult Tile-area impact, raider-defeat sound events, combat-defeat resource rewards, and in-game overlay menu.
...
- `internal/ui/` owns shared UI palette colors, font-size constants, text drawing helpers, simple widgets used by menu and game rendering, and generic selected-object panel presentation including labels, formatting, bounds, hit testing, and drawing.
...
3. The game package renders explored 15x15 Plots from its stored prototype map data. A new game starts with only the home Plot explored. Every current Plot stores the grasslands biome. The home Plot is open grassland containing the centered Sanctum as its only initial structure and a straight road north to the Plot edge. Magnifying-glass buttons appear on borders between explored and unexplored orthogonal Plots during calm play. Clicking one reveals a generated grasslands Plot with mostly buildable grass plus sparse Forest and Boulder Tiles; adjacent explored Plots have no plot-level frame, gutter, or padding between them; north-chain Plots also continue the straight center road. All combat towers must be built by the player.
...
6. Left-clicking a visible raider selects that raider; otherwise, left-clicking a structure tile selects that structure; clicking elsewhere clears selection. The selected object is stored in `internal/game.State`, rendered brighter, and adapted into UI-facing detail data for a bottom-right detail panel when supported. Panel clicks are treated as UI input and do not clear or pass through selection. Selection works while SPACE-paused, and the in-game overlay blocks selection changes.
7. The game package renders resources and available/total populations. Exploration buttons are handled as map-space controls but are blocked by screen-space UI. The building bar groups visible choices into `Housing`, `Economic`, and `Defenses` tabs, displays values to the right of each icon, shows informational tooltips when building icons are hovered, draws buildable icon slots with green outlines, draws capacity-blocked icon slots with red outlines and 70% icon opacity, and uses stable building identifiers for placement mapping. Building-bar eligibility requires sufficient resources and all required available roles. Successful staffed placement deducts resources and reserves staff atomically; successful House placement deducts Wood and grants Peasants by increasing both available and total counts; successful Barracks placement deducts Wood and Stone, removes Peasants from available and total counts, and grants Soldiers by increasing both available and total counts; successful Dorm placement deducts Wood and Stone, removes one Peasant from available and total counts, and grants one Apprentice by increasing both available and total counts; successful economic building placement reserves one available Peasant and later produces resources after defeated Raids. A new game starts at `0/0` for every role, so House is initially eligible but Barracks, Dorm, economic buildings, and towers are not.
8. Left-dragging an eligible building-bar icon during calm play starts a private build drag. A half-sized copy of the icon follows the cursor. Releasing over an empty `terrainEmpty` Tile with no feature in any explored Plot places the matching structure feature and applies its construction cost, population cost, staff reservation, and population grant; releasing over occupied Tiles, roads, forests, Boulders, unexplored space, during active Raids, or after breach cancels without spending resources or population effects. SPACE-paused calm play still permits placement, while the in-game overlay blocks building input.
9. While unpaused, each Ebitengine update advances the logical update counter by one. If a Raid is active, the game package advances deterministic enemy spawning, tower targeting and projectile combat, and enemy movement along the current central north road. Raid 1 spawns skeleton, zombie, skeleton, zombie, skeleton, while later placeholder Raids remain skeleton-only. Enemies spawn at the farthest explored north-chain Plot and move south toward the Sanctum. Enemies store Sanctum-centered world coordinates in Tile units, read Tiles-per-second movement speed and defeat-resource rewards from their enemy template, have health, and render from their template sprite in the active asset catalog. Combat towers in any explored Plot store timing stats in seconds, track cooldowns as seconds remaining, launch projectiles at in-range enemies, and damage enemies on projectile impact. Bow and Flame Bolt projectiles damage only their original target; Catapult projectiles damage every active enemy in the Tile occupied by the original target when the projectile lands. Tower-damage defeats grant the defeated raider's template resources and report a raider-defeated sound event to the runtime sound sink. When all Raid enemies are gone, placed Woodcutters, Stone Quarries, and Iron Mines in any explored Plot each add 10 Wood, Stone, or Metal respectively. Enemies that reach the Sanctum spend Barricade charges and are removed without rewards, or breach the Sanctum when no charges remain. Selected raider state is cleared when that raider is no longer active.
...
- Keep explored Plot map storage and reveal orchestration in `internal/game/map.go`, biome-backed Plot generation in `internal/game/plot_generator.go`, and the first camera/projection behavior, including right-drag camera state, in `internal/game/camera.go` until richer multi-map behavior or multiple gameplay scenes create a reason for separate packages.
...
To add gameplay systems, start with pure data and functions that can be tested by `go test ./...`. `internal/game/structures.go` owns staffing requirements, population-cost, population-grant, resource-yield, and description template metadata, `internal/game/hud.go` owns population availability, population costs, reservation, and grants, `internal/game/building_bar.go` owns building-bar rendering, tab input, and drag behavior, `internal/game/building_bar_items.go` owns stable building-bar item and category mapping, `internal/game/building_bar_metadata.go` owns compact cost and population metadata display helpers, `internal/game/building_tooltip.go` owns detailed hover tooltip formatting and rendering, and `internal/game/raid.go` applies economic building resource yields after successful Raid completion. A future removal or reassignment system must return reserved staff without duplicating template requirements, and must define how removing a population-provider, converter, or economic building affects inhabitants and post-Raid income before changing House, Barracks, or economic building behavior.
