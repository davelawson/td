...
7. The game package renders resources and available/total populations. The building bar groups visible choices into `Housing`, `Economic`, and `Defenses` tabs, displays values to the right of each icon, shows informational tooltips when building icons are hovered, draws buildable icon slots with green outlines, draws capacity-blocked icon slots with red outlines and 70% icon opacity, and uses stable building identifiers for placement mapping. Building-bar eligibility requires sufficient resources and all required available roles. Successful staffed placement deducts resources and reserves staff atomically; successful House placement deducts Wood and grants Peasants by increasing both available and total counts; successful Barracks placement deducts Wood and Stone, removes Peasants from available and total counts, and grants Soldiers by increasing both available and total counts; successful economic building placement reserves one available Peasant and later produces resources after defeated Raids. A new game starts at `0/0` for every role, so House is initially eligible but Barracks, economic buildings, and towers are not.
...
- Keep the first map data in `internal/game/map.go` and the first camera/projection behavior, including right-drag camera state, in `internal/game/camera.go` until real exploration, generation, multi-map behavior, or multiple gameplay scenes create a reason for separate packages.
