...
Documentation changes that are part of the feature are also subject to validation. If the ExecPlan includes updates to `PRODUCT.md`, `ROADMAP.md`, `CODESTYLE.md`, `ARCHITECTURE.md`, or another in-scope root-level `ALLCAPS.md` file, specify what a reviewer should check to confirm those files match the implemented behavior and remain consistent with the code layout and user-visible result.
