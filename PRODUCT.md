...
- Starting a new game only opens the first static home Plot scene; exploration, resource gathering, tower upgrades, broader base-building, and rewards are not implemented. The left building bar can spend resources only for this first tower placement action. The top bar still uses fixed prototype Chapter, calm-time, resource, and Barricade starting values until those systems exist, but resource labels are represented by icons and it uses real remaining enemy counts during active Raids.
