...
`Core`: The repository has a Go module, an Ebitengine executable under `cmd/td/`, a typed runtime asset catalog under `assets/`, pure menu behavior under `internal/menu/`, pure game-update behavior under `internal/game/`, static prototype map data and camera state under `internal/game/`, deterministic placeholder Raid state under `internal/game/`, and Go tests for asset loading, menu hit testing, disabled menu targets, action selection, screen routing, Wizard name input behavior, app mode startup, game update counting, pause behavior, camera zoom and pan behavior, in-game menu behavior, top-bar status formatting, Raid starts, staggered enemy spawning, enemy movement, Barricade spending, breach handling, and default home Plot invariants including the road opening through the tree border and the starting Bow Tower beside the path.

### Missing Gameplay And Operations

`Core`: There is currently no tower targeting, combat damage, exploration, base-building, resource gathering, asset pipeline, save system, settings implementation, campaign system, CI pipeline, license, or release packaging.
...
- The current playable shell intentionally includes only a small menu flow, Wizard name entry, a first game screen with a static tree-bordered home Plot scene, sprite-backed Sanctum, one non-combat Bow Tower beside the road, basic camera zoom and pan, deterministic placeholder Raids on the fixed north road, a logical update counter, pause behavior, an in-game overlay menu, surrender-to-menu behavior, and quit behavior.
- Saving the game and campaign structure are explicit non-goals for the first phase.
- Starting a new game only opens the first static home Plot scene; exploration, base-building, real resource changes, tower targeting, and combat damage are not implemented. The top bar still uses fixed prototype Chapter, calm-time, resource, and Barricade starting values until those systems exist, but it uses real remaining enemy counts during active Raids.
