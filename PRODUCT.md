...
- The current playable shell intentionally includes only a small menu flow, Wizard name entry, a first game screen with a static tree-bordered home Plot scene, sprite-backed Sanctum, automated Bow and Flame Bolt Towers beside the road, basic camera zoom and pan, deterministic placeholder Raids with sprite-backed skeleton and zombie enemies and health bars on the fixed north road, first-pass projectile combat, a logical update counter, pause behavior, an in-game overlay menu, surrender-to-menu behavior, and quit behavior.
