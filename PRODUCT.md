...
`Core`: The repository has a Go module, an Ebitengine executable under `cmd/td/`, pure menu behavior under `internal/menu/`, pure game-update behavior under `internal/game/`, static prototype map data under `internal/game/`, and Go tests for menu hit testing, disabled menu targets, action selection, screen routing, Wizard name input behavior, app mode startup, game update counting, pause behavior, in-game menu behavior, top-bar status formatting, and default home Plot invariants.
...
- The current playable shell intentionally includes only a small menu flow, Wizard name entry, a first game screen with a static home Plot scene, a logical update counter, pause behavior, an in-game overlay menu, surrender-to-menu behavior, and quit behavior.
