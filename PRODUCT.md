...
`Core`: The repository has a Go module, an Ebitengine executable under `cmd/td/`, typed runtime assets, menu and game packages, a Sanctum-only starting Plot, status state, camera and selection behavior, a widened tabbed building bar with right-side construction costs, population-grant metadata, staffing metadata, informational hover tooltips, green/red capacity outlines, capacity opacity feedback, drag placement, deterministic Raids, and Bow, Flame Bolt, and Catapult projectile combat. Tests cover these behaviors without depending on free starting towers.
