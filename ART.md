...
The Iron Mine structure sprite lives at `assets/sprites/structures/iron-mine.png`. It was generated as a centered low-detail medieval mine entrance with timber supports, a winch, and bluish iron ore on a flat chroma-key background, converted to alpha, cropped to subject bounds, and resized to 64x64 PNG.

The Market structure sprite lives at `assets/sprites/structures/market.png`. It was generated as one compact medieval timber stall with a blue cloth canopy and contained trade goods, using the existing economic structures as style references. A flat `#ff00ff` background was removed locally before nearest-neighbor normalization onto an exact 64x64 transparent canvas. Review the stall silhouette and canopy at both 64x64 and 32x32.
...
The Gold resource icon lives at `assets/sprites/icons/gold.png`. It contains exactly two golden coin stacks, one taller than the other, with no loose coins or other objects. It was generated against flat `#ff00ff`, removed to alpha, nearest-neighbor normalized onto a 64x64 transparent canvas, and reviewed at the 28-pixel top-HUD size and 32x32 evidence size. Future currency icons should preserve the existing resource icons' compact hard-cluster pixel style and uncluttered silhouette.
...
The Ghoul and Armoured Skeleton enemy sprites live at `assets/sprites/enemies/ghoul.png` and `assets/sprites/enemies/armoured-skeleton.png`. They use the existing Skeleton and Zombie sprites as style, scale, pixel-density, and silhouette references. Both were generated on flat `#ff00ff`, converted to alpha with automatic border sampling, soft matte, and despill, cropped to their visible subjects, resized with nearest-neighbor sampling, and centered on exact 64x64 transparent PNG canvases. The Ghoul uses a narrow hunched corpse-pale runner silhouette with long separated claws; the Armoured Skeleton uses a broad dark-steel silhouette with a visible skull, helmet, pauldrons, breastplate, gauntlets, and greaves. Future enemies should remain isolated, fully contained, free of scenery, props, ground patches, shadows, text, and key-color fringe. Review silhouette and role identifiers at both 64x64 and 32x32 before accepting an enemy sprite.
